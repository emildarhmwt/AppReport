...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($username)) {
     ...
     if (!empty($_FILES['file_path']['name'])) {
        ...
        $query = "SELECT file_admin FROM user_report WHERE id = $1";
    ...
    if (!empty($password)) {
        ...
        $query = "UPDATE user_report SET nama = $1, username = $2, password = $3, file_admin = $4 WHERE id = $5";
        $result = pg_query_params($conn, $query, array($nama, $username, $hashedPassword, $filePath, $id));
    } else {
        // Update without changing the password
        $query = "UPDATE user_report SET nama = $1, username = $2, file_admin = $3 WHERE id = $4";
        $result = pg_query_params($conn, $query, array($nama, $username, $filePath, $id));
    }

    if ($result) {
        echo "User berhasil diperbarui.";
        header("Location: User.php"); // Redirect to the admin list page
        exit;
    } else {
