...
<body>
            ...
            paginatedData.forEach((report, index) => {
                ...
                <tr class="text-center">
                    ...
                    <div class="d-flex justify-content-between">
                        <button class="btn btn-warning btn-sm me-2" title="Edit" onclick="window.location.href='editoperationhm.php?id=${report.id}'"><i class="bi bi-pencil-square"></i></button>
                        <button class="btn btn-danger btn-sm" title="Delete" onclick="deleteOperation(${report.id})"><i class="bi bi-trash"></i></button>
                    </div>
                        <button onclick="window.location.href='Preview_hm.php?id=${report.id}'" class="btn btn-primary btn-sm mt-1" title="Edit"><i class="bi bi-eye"></i></button>
                    </td>
                </tr>`;
                tbody.innerHTML += row;
            });
    ...
    function deleteOperation(id) {
        if (confirm(
                'Apakah Anda yakin ingin menghapus data ini? Jika data ini dihapus, data yang terkait dalam laporan jam jalan juga akan terhapus.'
            )) {
            fetch('Report_hourmeter.php', {
                    method: 'POST',
                    headers: {
