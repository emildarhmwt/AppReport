...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['file_path']['name'])) {
        ...
        $query = "SELECT file_path FROM barcode_kontraktor WHERE id = $1";
        $result = pg_query_params($conn, $query, array($id));
        $row = pg_fetch_assoc($result);
        $filePath = $row['file_path'];
    }

    $query = "UPDATE barcode_kontraktor SET nama = $1, jabatan = $2, nip = $3, file_path = $4 WHERE id = $5";
    $result = pg_query_params($conn, $query, array($nama, $jabatan, $nip, $filePath, $id));

    if ($result) {
        echo "Barcode kontraktor berhasil diperbarui.";
        header("Location: Barcode_kontraktor.php");
        exit;
    } else {
        echo "Terjadi kesalahan saat memperbarui barcode kontraktor.";
    }
} else {
