...
$sql_production = "
    ...
    JOIN operation_report o ON p.operation_report_id = o.id
    WHERE p.proses_admin = 'Uploaded' 
    AND p.proses_pengawas IS NULL 
    AND o.pic = $1
";

$sql_hourmeter = "
    SELECT COUNT(*) 
    FROM hourmeter_report h
    JOIN operation_report o ON h.operation_report_id = o.id
    WHERE h.proses_admin = 'Uploaded' 
    AND h.proses_pengawas IS NULL 
    AND o.pic = $1
";
...
$sql_production_kontraktor = "
    SELECT COUNT(*) 
    FROM production_report p
    JOIN operation_report o ON p.operation_report_id = o.id
    WHERE p.proses_kontraktor = 'Rejected Kontraktor' 
    AND o.pic = $1
";

$sql_hourmeter_kontraktor = "
    SELECT COUNT(*) 
    FROM hourmeter_report h
    JOIN operation_report o ON h.operation_report_id = o.id
    WHERE h.proses_kontraktor = 'Rejected Kontraktor' 
    AND o.pic = $1
";

$rejected_production_kontraktor = pg_fetch_result(pg_query_params($conn, $sql_production_kontraktor, array($_SESSION['username'])), 0, 0);
$rejected_hourmeter_kontraktor = pg_fetch_result(pg_query_params($conn, $sql_hourmeter_kontraktor, array($_SESSION['username'])), 0, 0);

$total_rejected_kontraktor = $rejected_production_kontraktor + $rejected_hourmeter_kontraktor;
                        ...
                        <div class="card bg-selamat" style="height: 77%;">
                            <div class="card-body">
                                ...
                                <h5 class="kamu"> Anda memiliki <?php echo $total_rejected; ?> berkas yang menunggu
                                    persetujuan dan <?php echo $total_rejected_kontraktor; ?> berkas yang telah ditolak
                                    kontraktor. Mohon
                                    segera diperiksa.</h5>
